...
const noiseOverlayStyle = {
  position: 'absolute', inset: 0, borderRadius: 'inherit',
  backgroundImage: noiseDataUrl, backgroundSize: '200px 200px',
  opacity: 0.4, mixBlendMode: 'overlay', pointerEvents: 'none', zIndex: 2,
};
...
const rimGlowStyle = {
  ...
  borderRadius: '50%', filter: 'blur(0.5px)', pointerEvents: 'none', zIndex: 3,
};

const inputStyle = {
  width: '100%', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.2)',
  borderRadius: '6px', color: 'white', padding: '6px 10px', fontSize: '13px', outline: 'none', fontFamily: 'inherit'
};

const actionBtnStyle = {
  background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)',
  color: 'white', padding: '4px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em'
};
...
function TypeBadge({ label }) {
  return (
    <span style={{
      display: 'inline-block', padding: '3px 10px', borderRadius: 999, fontSize: 11, fontWeight: 600, letterSpacing: '0.04em',
      background: 'linear-gradient(135deg, rgba(99,179,255,0.18), rgba(120,100,255,0.12))', border: '0.5px solid rgba(130,190,255,0.3)',
      color: 'rgba(160,210,255,0.95)', backdropFilter: 'blur(8px)', boxShadow: '0 1px 0 rgba(255,255,255,0.1) inset',
    }}>
      {label || 'Unknown'}
    </span>
  );
...
export default function App() {
  ...
  const analyzeQuote = async () => {
    ...
    try {
      ...
      // SAFETY CHECK: Ensure data is not null before setting it or reading it
      if (data) {
        setResults(data);
  ...
  return (
    <div style={{
      ...
      color: 'white', padding: '60px 24px 80px', boxSizing: 'border-box',
    }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <header style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <h1 style={{
            ...
            background: 'linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(180,200,255,0.7) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
          }}>
            Invoice Analyzer
          </h1>
          ...
          <GlassCard style={{ padding: 36 }}>
            <label
              ...
              style={{
                ...
                height: 140, borderRadius: 16, border: `1.5px dashed ${dragOver ? 'rgba(100,170,255,0.6)' : 'rgba(255,255,255,0.12)'}`,
                background: dragOver ? 'rgba(80,140,255,0.08)' : 'rgba(255,255,255,0.02)', cursor: 'pointer', transition: 'all 0.2s ease',
              }}
            >
              <div style={{
                ...
                border: '0.5px solid rgba(140,180,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 0 rgba(255,255,255,0.15) inset',
              }}>
            ...
            <button
              ...
              disabled={loading}
              style={{
                marginTop: 24, width: '100%', padding: '14px 32px', borderRadius: 14, border: '0.5px solid rgba(255,255,255,0.2)',
                background: loading ? 'rgba(60,100,200,0.3)' : 'linear-gradient(135deg, rgba(70,130,255,0.55) 0%, rgba(100,60,220,0.45) 100%)',
                backdropFilter: 'blur(12px)', color: 'white', fontWeight: 700, fontSize: 15, letterSpacing: '0.01em',
                cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1,
                boxShadow: loading ? 'none' : `0 1px 0 rgba(255,255,255,0.25) inset, 0 -1px 0 rgba(0,0,0,0.2) inset, 0 4px 20px rgba(60,100,255,0.3), 0 1px 4px rgba(0,0,0,0.3)`,
            ...
            >
              {loading ? (
                <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                  <span style={{
                    display: 'inline-block', width: 16, height: 16, border: '2px solid rgba(255,255,255,0.25)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.7s linear infinite',
                  }} />
                  Processing…
                </span>
              ) : 'Analyze Invoice'}
        ...
        {results?.items?.length > 0 && (
          <div style={{ marginTop: 32 }}>
            <GlassCard style={{ padding: 0, overflow: 'hidden' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1150 }}>
                  <thead>
                    <tr style={{ background: 'rgba(255,255,255,0.03)' }}>
                      {['Item', 'HSN/SAC', 'Type', 'Sub-Type', 'Tax %', 'Qty', 'Unit Price', 'Base Total', 'Total (w/ Tax)', 'Actions'].map((h, i) => (
                        <th key={h} style={{
                          padding: '12px 16px', fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', color: 'rgba(140,170,220,0.6)', textTransform: 'uppercase',
                          textAlign: (i >= 4 && i <= 8) ? 'right' : (i === 9 ? 'center' : 'left'), borderBottom: '0.5px solid rgba(255,255,255,0.06)',
                        }}>
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {results.items.map((item, i) => {
                      ...
                      return (
                        <tr key={i} style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)', transition: 'background 0.15s', background: isEditing ? 'rgba(255,255,255,0.05)' : 'transparent' }}
                        ...
                        >
                          ...
                          <td style={{ padding: '12px 16px', textAlign: 'right', fontSize: 13, color: 'rgba(180,200,240,0.7)', minWidth: '80px' }}>
                            {isEditing ? <input style={{...inputStyle, textAlign: 'right'}} value={editFormData.tax_percentage || ''} onChange={(e) => handleEditChange('tax_percentage', e.target.value)} /> : (item.tax_percentage ? `${getTaxRate(item.tax_percentage)}%` : '-')}
                          </td>
                          ...
                          <td style={{ padding: '12px 16px', textAlign: 'right', color: 'rgba(160,170,210,0.6)', fontSize: 13, minWidth: '90px' }}>
                            {isEditing ? <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>Auto-Calc</span> : `₹${item.unit_price?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                          </td>
                          <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 600, color: 'rgba(200,210,240,0.85)', fontSize: 14, minWidth: '110px' }}>
                            {isEditing ? <input type="number" style={{...inputStyle, textAlign: 'right'}} value={editFormData.amount || ''} onChange={(e) => handleEditChange('amount', e.target.value)} /> : `₹${(item.amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                          </td>
                          <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 700, color: 'rgba(240,245,255,0.95)', fontSize: 14, minWidth: '110px' }}>
                            {isEditing ? <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>Auto-Calc</span> : `₹${getItemTotalWithTax(item).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                          </td>
              ...
              <div style={{ padding: '20px 24px', borderTop: '0.5px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10, background: 'rgba(255,255,255,0.015)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginTop: 24, gap: 16 }}>
              ...
              <button
                ...
                disabled={savingDb}
                style={{
                  padding: '12px 28px', borderRadius: 12, border: '0.5px solid rgba(80, 200, 120, 0.4)',
                  background: savingDb ? 'rgba(80, 200, 120, 0.1)' : 'linear-gradient(135deg, rgba(60, 180, 100, 0.2) 0%, rgba(40, 140, 80, 0.1) 100%)',
                  color: savingDb ? 'rgba(255,255,255,0.5)' : 'white', fontWeight: 700, fontSize: 14, cursor: savingDb ? 'not-allowed' : 'pointer',
                  transition: 'all 0.2s ease', boxShadow: '0 4px 12px rgba(40, 160, 80, 0.15)',
                }}
              >
