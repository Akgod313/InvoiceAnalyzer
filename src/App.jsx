...
function App() {
  ...
  const analyzeQuote = async () => {
    ...
    try {
      // MAKE SURE TO PUT YOUR REAL RENDER URL HERE
  ...
  return (
    <div className="min-h-screen bg-[#0b0f1a] text-white font-sans selection:bg-blue-500/30 pb-20">
      {/* Background Glows */}
        ...
        <div className="relative group max-w-2xl mx-auto">
          ...
          <div className="relative bg-[#161b2c]/80 backdrop-blur-xl border border-white/10 p-12 rounded-[2rem] shadow-2xl flex flex-col items-center">
            ...
            <button 
              ...
              disabled={loading}
              className="mt-8 px-12 py-4 bg-blue-600 rounded-full font-bold text-lg shadow-[0_0_20px_rgba(37,99,235,0.4)] hover:shadow-[0_0_30px_rgba(37,99,235,0.6)] hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto"
            >
              {loading ? (
                  ...
                  {results.items.map((item, i) => (
                    <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="p-5 text-xs text-gray-400">
                        <span className="px-2 py-1 rounded-md bg-blue-500/10 text-blue-400 border border-blue-500/20 whitespace-nowrap">
                          {item.type || "Unknown"}
                        </span>
                      </td>
                      <td className="p-5 text-xs text-gray-400 max-w-[150px] truncate" title={results.paid_to}>
                        {results.paid_to || "N/A"}
                      </td>
                      <td className="p-5 text-[10px] font-mono text-gray-500 max-w-[120px] truncate" title={results.gstin_numbers?.join(", ")}>
                        {results.gstin_numbers?.join(", ") || "N/A"}
                      </td>
                      <td className="p-5 text-center font-bold text-blue-400">{item.quantity}</td>
                      <td className="p-5 text-right text-gray-400 whitespace-nowrap">₹{item.unit_price?.toLocaleString()}</td>
                      <td className="p-5 text-right font-bold text-white whitespace-nowrap">₹{item.amount?.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
